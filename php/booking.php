...
<body>
  ...
  <main>
    ...
    <div class="container">
      ...
      // Get the selected venue from the URL parameters
      $selectedVenue = isset($_GET['venue']) ? htmlspecialchars($_GET['venue']) : 'No venue selected';
      ...
      <!-- Display the selected venue -->
      <h2>Booking for: <?php echo $selectedVenue; ?></h2>

      <form action="process_booking.php" method="POST">
        <!-- Hidden input to store the selected venue -->
        <input type="hidden" name="venue" value="<?php echo $selectedVenue; ?>">
        ...
        <select name="booking_time" id="booking_time" onchange="getFieldStatus()" required>
          <?php
...
<script>
  function getFieldStatus() {
    ...
    // Get the selected date and time
    var selectedTimeValue = document.getElementById("booking_time").value;

    // Create an XMLHttpRequest object
    var xhr = new XMLHttpRequest();

    // Set up the AJAX request
    xhr.open("GET", "get_field_status.php?booking_date=" + selectedDate + "&booking_time=" + selectedTimeValue, true);
